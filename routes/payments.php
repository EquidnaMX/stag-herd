...
if (config('stag-herd.payments.routes.enabled', true)) {
    Route::middleware(config('stag-herd.payments.routes.middleware', ['api']))
        ->prefix(config('stag-herd.payments.routes.prefix', config('stag-herd.demo.prefix', 'stag-herd/payments')))
        ...
        ->group(function (): void {
            ...
            if (config('stag-herd.providers.stripe.enabled', false)) {
                Route::prefix('stripe')->name('stripe.')->controller(StripeController::class)->group(function (): void {
                    ...
                    Route::post('/spei', 'processSpei')->name('spei');
