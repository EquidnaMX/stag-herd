...
function resolveClientSecret(response: StripeIntentResponse): string | null {
  ...
  const clientSecret = metadata.stripe_client_secret ?? response.client_secret;
...
function resolvePaymentIntentId(response: StripeIntentResponse): string | null {
  ...
  const providerPaymentId = response.payment?.references?.provider_payment_id;

  return typeof providerPaymentId === "string" && providerPaymentId !== ""
    ? providerPaymentId
    : null;
}
...
function StripeCardForm({
  ...
  async function handleSubmit(
    ...
    if (!stripe || !elements)
      return changeStatus("error", "Stripe todavía no está listo.");
    if (!ready)
      return changeStatus(
        "error",
        "Espera a que cargue el formulario de Stripe.",
      );
    ...
    if (!Number.isFinite(numericAmount) || numericAmount <= 0) {
      return changeStatus("error", "El monto no es válido.");
    }

    if (!currency.trim()) {
      return changeStatus("error", "La moneda es obligatoria.");
    }

    const cardElement = elements.getElement(CardElement);

    if (!cardElement) {
      return changeStatus("error", "No se pudo cargar el campo de tarjeta.");
    }
    ...
    try {
      ...
      if (!clientSecret) throw new Error("Stripe no regresó client_secret.");
      if (!paymentIntentId)
      ...
      changeStatus(
        "error",
        getErrorMessage(error, "No se pudo completar el pago con Stripe."),
      );
      onError?.(error);
    } finally {
