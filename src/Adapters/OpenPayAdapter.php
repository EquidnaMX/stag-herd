...
class OpenPayAdapter implements OpenpayGateway
{
    ...
    {
        $this->merchantId = (string) config('stag-herd.openpay.merchant_id');

        $this->privateKey = (string) config('stag-herd.openpay.private_key');

        $this->sandbox = (bool) config('stag-herd.openpay.sandbox', true);
    ...
    public function createBankCharge(
        ...
        if (!$response->successful()) {
            Log::channel(config('stag-herd.audit_log_channel', 'stack'))->error('Openpay charge creation failed', [
    ...
    public function getChargeDetails(string $chargeId): object
    {
        ...
        if (!$response->successful()) {
            Log::channel(config('stag-herd.audit_log_channel', 'stack'))->error('Failed to get Openpay charge details', [
                'status' => $response->status(),
            ]);
    ...
    public function getRefund(
        ...
        if (!$response->successful()) {
            Log::channel(config('stag-herd.audit_log_channel', 'stack'))->error('Openpay refund failed', [
                'status' => $response->status(),
            ]);
