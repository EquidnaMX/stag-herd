 ...
 * Conekta and Openpay. Includes idempotency checking to prevent duplicate processing.
 *
 * PHP 8.1+
 *
...
class WebhookVerifier
{
    ...
    public static function verifyMercadoPagoSignature(Request $request, string $secret): array
    {
        try {
            ...
            $dataId = is_string($dataId) ? $dataId : '';
            if ($dataId === '') {
            ...
            return ['valid' => true, 'eventId' => $dataId];
        } catch (Exception $e) {
            return ['valid' => false, 'reason' => $e->getMessage()];
        }
    }
    ...
    public static function verifyConektaSignature(Request $request): array
    {
        try {
            ...
            $publicKey = config('stag-herd.conekta.webhook_public_key');
            if (!is_string($publicKey) || trim($publicKey) === '') {
                return ['valid' => false, 'reason' => 'Missing Conekta webhook public key'];
            }
            $publicKey = str_replace('\n', "\n", $publicKey);

            $providedDigest = str_starts_with($digest, 'sha-256=')
                ? substr($digest, 8)
                : $digest;

            $signature = base64_decode($providedDigest, true);
            if ($signature === false) {
                return ['valid' => false, 'reason' => 'Invalid Digest encoding'];
            }

            $payload = $request->getContent();
            $key = openssl_pkey_get_public($publicKey);
            if ($key === false) {
                return ['valid' => false, 'reason' => 'Invalid Conekta public key'];
            }
    ...
    public static function verifyOpenpaySignature(Request $request, string $secret): array
    {
        try {
            ...
            // Openpay signature: HMAC-SHA256(timestamp.data, secret)
            $signedPayload = $ts . '.' . $data;
            $computed = hash_hmac('sha256', $signedPayload, $secret);
            ...
            return ['valid' => true, 'eventId' => $payload['id'] ?? $payload['event_id'] ?? null];
        } catch (Exception $e) {
            return ['valid' => false, 'reason' => $e->getMessage()];
        }
    }
    ...
    public static function checkIdempotency(string $eventId, string $provider = 'test', int $ttl = 300): bool
    {
        ...
        return !self::isIdempotentAndStore(
            provider: $provider,
            eventId: $eventId,
            ttl: $ttl,
        );
